...
export default class SaveTabs
{
	...
	setSelectedTabGroup(selectedTabGroup: string)
	{
	 ...
	 */
	_saveTabs(tabQuery: Parameters<typeof browser.tabs.query>[0])
	{
		this._loadSettings()
			.then(() => browser.tabs.query(tabQuery))
			.then(this._collectUrls.bind(this))
			.then(this._addStoredUrls.bind(this))
			.then(urls => urls.filter((elem: string, index: number, arr: any[]) => arr.indexOf(elem) == index)) //remove duplicate URLs
	 ...
	 */
	_collectUrls(tabs: browser.tabs.Tab[])
	{
		let urls: string[] = [];
		for (let tab of tabs)
		{
			if (tab.url && this._isSaveable(tab))
			{
				urls.push(tab.url);
			}
		}
	...
	_migrateOldStorage()
	{
		 ...
		 */
		function migrator(this: SaveTabs, storage: any)
		{
			...
			if (storage[SAVED_TABS_KEY])
			{
				...
				if (existingData && Array.isArray(existingData) && existingData.length > 0)
				{
					...
					key = 'Migration-' + browser.runtime.id
		 ...
		 */
		function migrate(this: SaveTabs) {
			return browser.storage.local.get()
				.then(data => migrator.call(this, data))
	 ...
	 * if it is not open.
	 */
	_isOpen(openTabs: browser.tabs.Tab[], url: string)
	{
	 ...
	 */
	_isSaveable(tab: browser.tabs.Tab)
	{
		if (tab.pinned)
		{
		...
		if (!tab.url || NON_SAVEABLE_URLS.includes(tab.url))
		{
			return false;
		}
