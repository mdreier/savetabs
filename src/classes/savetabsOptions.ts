...
export default class SaveTabsOptions {
    ...
    _mergeSettings(settings: SaveTabsSettings): SaveTabsSettings {
      const settingsClone = settings as SaveTabsSettingsClone
      for (const key in DEFAULT_SETTINGS) {
        if (DEFAULT_SETTINGS.hasOwnProperty(key) && !settings.hasOwnProperty(key)) {
          const settingsKey = key as keyof SaveTabsSettings
     ...
     */
    _cloneSettings() {
      const clone = {} as SaveTabsSettingsClone
      for (const prop in this._settings) {
        if (this._settings.hasOwnProperty(prop)) {
          const myprop = prop as keyof SaveTabsSettings
